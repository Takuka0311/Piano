...
- 已支持多级运行时回退链：`wasapi -> dsound -> log`。
- 已新增 GUI Alpha（`piano_gui`）：播放控制、文件选择、参数配置、键盘可视化、配置持久化。
- 已统一错误码前缀，并补齐后端优先级与最近文件持久化。
...
`wasapi/dsound` 模式会按事件触发音符；`log` 模式会输出调度日志（NoteOn/NoteOff、TempoChange、TransposeChange），用于回归验证。
...
## 下一步（M6）
- 进入 M6：MIDI Out / VSTi 能力接入与验证。
- 继续完善音频质量（包络、失真控制、延迟优化）。
- 增加导出与可观测能力（M7）。
